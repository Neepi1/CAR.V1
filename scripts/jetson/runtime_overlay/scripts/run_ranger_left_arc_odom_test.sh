...
COUNTDOWN_SEC="3"
SETTLE_SEC="3.0"
LABEL="left_arc_r1p5_360"
CMD_TOPIC="/cmd_vel_collision_checked"
ODOM_TOPIC="/wheel/odom"
LOCAL_ODOM_TOPIC="/local_state/odometry"
OUTPUT_ROOT="${PROJECT_ROOT}/reports/ranger_left_arc_odom_test"
...
usage() {
  cat <<'EOF'
Usage: run_ranger_left_arc_odom_test.sh [options]

Runs an automated Ranger Mini 3 left Ackermann arc/circle odometry test. The
script publishes linear.x plus positive angular.z through the normal safety
chain and stops by accumulated /wheel/odom yaw.

Default profile:
  radius=1.5m, linear_speed=0.25m/s, angle=360deg left turn

Options:
  --radius-m M            Commanded turn radius. Default: 1.50
  --linear-speed MPS      Forward speed command. Default: 0.25
  --angle-deg DEG         Left-turn accumulated yaw target. Default: 360.0
  ...
  --label NAME            Report label. Default: left_arc_r1p5_360
  ...
  --output-root DIR       Report root. Default: reports/ranger_left_arc_odom_test
...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "[ranger-left-arc-test] unknown argument: $1" >&2
      usage >&2
      exit 2
      ;;
  esac
...
{
  echo "# Ranger Left Arc Odom Test Environment"
...
if [[ "${COUNTDOWN_SEC}" != "0" ]]; then
  echo "[ranger-left-arc-test] motion starts in ${COUNTDOWN_SEC}s. Ensure the left-turn circle envelope is clear and E-stop is available."
  remaining="${COUNTDOWN_SEC}"
  while [[ "${remaining}" -gt 0 ]]; do
    echo "[ranger-left-arc-test] ${remaining}..."
    sleep 1
...
commanded_wz = linear_speed / radius_m
timeout_sec = target_yaw / commanded_wz + max_extra_sec
...
class SegmentResult:
    ...
    local_peak_left_m: Optional[float]
...
class LeftArcNode(Node):
    def __init__(self) -> None:
        super().__init__("ranger_left_arc_odom_test")
...
def run_segment(node: LeftArcNode, index: int, writer: csv.DictWriter) -> SegmentResult:
    ...
    local_peak_left: Optional[float] = 0.0 if start_local else None
    ...
    while rclpy.ok():
        ...
        if node.wheel_odom is not None:
            ...
            wheel_peak_left = max(wheel_peak_left, wheel_left)
        ...
        if node.cmd_out is not None:
            ...
            if abs(node.cmd_out.linear.x) >= linear_speed * 0.4 and abs(node.cmd_out.angular.z) >= commanded_wz * 0.4:
    ...
    return SegmentResult(
        ...
        wheel_peak_left_m=wheel_peak_left,
        ...
        local_peak_left_m=local_peak_left,
    ...
    try:
        ...
        try:
            with open(os.path.join(out_dir, "summary.md"), "w", encoding="utf-8") as f:
                f.write("# Ranger Left Arc Odom Test Summary\n\n")
                ...
                expected_peak_left = radius_m * (1.0 - math.cos(min(target_yaw, math.pi)))
                f.write(f"- expected_peak_left_m: `{expected_peak_left:.3f}`\n")
                ...
                f.write("\n| segment | ok | reason | duration_sec | wheel_yaw_deg | wheel_yaw_error_deg | wheel_forward_m | wheel_left_m | wheel_closure_m | wheel_peak_left_m | wheel_peak_left_error_m | wheel_effective_radius_m | local_yaw_deg | local_yaw_error_deg | local_forward_m | local_left_m | local_closure_m | local_peak_left_m | max_cmd_safe_vx | max_cmd_safe_wz | max_cmd_out_vx | max_cmd_out_wz | motion_modes_seen | system_modes_seen |\n")
                f.write("|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|\n")
                for r in results:
                    ...
                    local_peak_left = "" if r.local_peak_left_m is None else f"{r.local_peak_left_m:.4f}"
                    wheel_effective_radius = r.wheel_peak_left_m / 2.0
                    f.write(
                        ...
                        f"{r.wheel_forward_m:.4f} | {r.wheel_left_m:.4f} | {r.wheel_closure_m:.4f} | {r.wheel_peak_left_m:.4f} | "
                        f"{r.wheel_peak_left_m - expected_peak_left:.4f} | {wheel_effective_radius:.4f} | "
                        f"{local_yaw} | {local_yaw_error} | {local_forward} | {local_left} | {local_closure} | {local_peak_left} | "
...
rc=$?
set -e
echo "[ranger-left-arc-test] report: ${OUT_DIR}"
exit "${rc}"
