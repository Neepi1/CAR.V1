...
NODE_BIN="${NJRH_PROJECT_ROOT}/install/robot_global_localization/lib/robot_global_localization/global_localization_node"
[[ -x "${NODE_BIN}" ]] || {
...
if [[ -f "${PARAMS_FILE}" ]]; then
  njrh_exec_affined robot_global_localization "${NODE_BIN}" --ros-args --params-file "${PARAMS_FILE}"
fi

njrh_exec_affined robot_global_localization "${NODE_BIN}"
