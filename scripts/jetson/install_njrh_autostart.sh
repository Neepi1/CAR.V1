...
write_env_file_if_missing() {
  ...
  if [[ -f "${ENV_FILE}" ]]; then
    ...
    if sudo grep -q '^NJRH_AMCL_RESIDENT_WARMUP_BEFORE_INITIAL_LOCALIZATION=' "${ENV_FILE}"; then
      sudo sed -i 's/^NJRH_AMCL_RESIDENT_WARMUP_BEFORE_INITIAL_LOCALIZATION=.*/NJRH_AMCL_RESIDENT_WARMUP_BEFORE_INITIAL_LOCALIZATION=true/' "${ENV_FILE}"
    else
      echo 'NJRH_AMCL_RESIDENT_WARMUP_BEFORE_INITIAL_LOCALIZATION=true' | sudo tee -a "${ENV_FILE}" >/dev/null
    fi
    ...
    if sudo grep -q '^NJRH_NAV2_PRESTART_BEFORE_INITIAL_LOCALIZATION=' "${ENV_FILE}"; then
      sudo sed -i 's/^NJRH_NAV2_PRESTART_BEFORE_INITIAL_LOCALIZATION=.*/NJRH_NAV2_PRESTART_BEFORE_INITIAL_LOCALIZATION=true/' "${ENV_FILE}"
    else
      echo 'NJRH_NAV2_PRESTART_BEFORE_INITIAL_LOCALIZATION=true' | sudo tee -a "${ENV_FILE}" >/dev/null
    fi
