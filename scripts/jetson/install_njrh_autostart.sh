...
write_env_file_if_missing() {
  ...
  if [[ -f "${ENV_FILE}" ]]; then
    ...
    if grep -q '^NJRH_INITIAL_GLOBAL_LOCALIZATION_BACKGROUND_START=' "${ENV_FILE}"; then
    ...
    if grep -q '^NJRH_NAV2_PRESTART_BEFORE_INITIAL_LOCALIZATION=' "${ENV_FILE}"; then
    ...
    if grep -q '^NJRH_NAV2_LIFECYCLE_PARALLEL_CORE=' "${ENV_FILE}"; then
    ...
    if grep -q '^NJRH_PREPARE_RUNTIME_PERMISSIONS_MODE=' "${ENV_FILE}"; then
    ...
    if grep -q '^NJRH_COMMON_LOCAL_STATE_START_READY_MODE=' "${ENV_FILE}"; then
    ...
    if grep -q '^NJRH_NAV2_HELD_PRESTART_WAIT_FOR_LOCALIZER_SERVICE=' "${ENV_FILE}"; then
