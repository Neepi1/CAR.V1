...
- local dynamic-obstacle avoidance is tuned for Ranger Mini 3 differential drive: `collision_monitor` now reserves hard stop for close body-risk points, uses a weaker larger slowdown zone, and MPPI now samples more angular trajectories with lower path-alignment weight so blocked global paths can be locally skirted instead of immediately stalling.
