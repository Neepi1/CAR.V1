...
- Web dashboard floor controls are test-only: list floor assets, promote a saved map into a floor bundle, select a floor bundle for later Web-launched stacks, and call `/floor_manager/switch_floor`. They exercise the repository-owned floor contract during field testing and are not the production mission UI.
