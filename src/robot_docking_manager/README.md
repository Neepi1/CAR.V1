...
The field profile is intentionally conservative: GS2 yaw line-fitting is enabled, lateral and yaw error are low-pass filtered and deadbanded, and fine docking is staged by motion mode. If yaw is outside the soft tolerance, `controller.yaw_spin_priority_enabled=true` makes the node request official Ranger `spinning` mode and publish a yaw-only command. Once yaw is inside tolerance, the node treats yaw as locked, requests `side_slip` mode, and publishes `angular.z=0` so `linear.y` alone corrects lateral offset before the final insert. The GS2 lateral sign is inverted for the current physical mounting through `controller.lateral_command_sign=-1.0`; if a future mount reports `y` with the ROS `base_link` sign, change that value back to `1.0`. The yaw gain is signed for the current GS2 mounting as `controller.kyaw=0.70`; the live yaw-spin evidence showed positive GS2 fitted yaw must command positive body yaw in the current staged controller. Docking is staged: while lateral or yaw error is outside the priority threshold, `max_forward_while_lateral_mps=0.000` keeps the robot from advancing. The lateral soft tolerance is `3.0cm`, the yaw soft tolerance is `5deg`, and both `min_align_speed_mps=0.025` and `min_lateral_speed_mps=0.025` keep small residual corrections above the downstream Ranger command deadband. `/docking/status` reports `phase`, `forced_mode`, `cmd_vx`, `cmd_vy`, `cmd_wz`, `desired_contact_vy`, `pivot_comp_vy`, and `charge_contact_x` for field validation. Once centered and squared, `lock_lateral_during_final_insert=true` forces `linear.y=0` and `angular.z=0` so the final contact insert is straight forward only.
