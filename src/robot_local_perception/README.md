...
- current repository implementation now prefers a repo-owned C++ node for Jetson runtime stability and keeps the Python node only as a fallback path
...
- Jetson runtime executes `scripts/jetson/runtime_overlay/scripts/run_local_perception.sh`, which prefers the compiled `local_perception_node` binary and only falls back to the Python script if the package has not been built yet.
