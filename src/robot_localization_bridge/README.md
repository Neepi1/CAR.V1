...
- `amcl_max_result_age_ms`, `amcl_small_correction_translation_m`, `amcl_small_correction_yaw_rad`: AMCL-specific freshness and direct small-correction gates. The field profile directly accepts translation corrections up to `0.07 m` and yaw corrections up to `0.20 rad`.
...
- `amcl_post_isaac_refine_*`: after a force-accepted Isaac relocalization seeds AMCL, gated AMCL may apply one stationary, short-window residual correction before the normal post-Isaac suppression resumes. The field profile allows one correction within `10.0 s` of the successful AMCL seed, capped at `0.12 m` and `0.10 rad`, after `2` candidates that agree within `0.08 m` and `0.08 rad`.
...
- Isaac triggered relocalization has the highest priority and can seed AMCL through `/initialpose`. Immediately after an explicit Isaac accept, the bridge allows at most one stationary post-Isaac AMCL refine correction in gated mode so AMCL can remove small scan-map residuals from the coarse global relocalization. Outside that short refine window, AMCL gated corrections directly accept small covariance-gated updates, accept medium corrections only after consecutive consistency, and report large corrections for Isaac recovery instead of applying one-frame TF jumps.
