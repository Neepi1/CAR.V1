...
class RangerROSMessenger : public std::enable_shared_from_this<RangerROSMessenger>
{
 ...
 private:
  ...
  bool EnsureMotionModeReady(uint8_t desired_mode);
  void ResetModeSwitchState(uint8_t desired_mode);
  void PublishModeStatus();
  const char* ModeSwitchStateName() const;
  ...
  bool mode_switch_handshake_enabled_;
  double mode_switch_retry_period_sec_;
  double mode_switch_timeout_sec_;
  double mode_switch_stable_duration_sec_;
  double mode_switch_stop_linear_threshold_mps_;
  double mode_switch_stop_angular_threshold_radps_;
  std::string mode_status_topic_;
  std::string legacy_mode_status_topic_;
  ...
  double latest_feedback_linear_velocity_ = 0.0;
  ...
  bool desired_motion_mode_valid_ = false;
  uint8_t desired_motion_mode_ = 0;
  bool mode_switch_active_ = false;
  bool mode_switch_stop_stable_ = false;
  bool mode_switch_request_sent_ = false;
  uint8_t mode_switch_target_ = 0;
  ModeSwitchState mode_switch_state_ = ModeSwitchState::kStable;
  std::chrono::steady_clock::time_point mode_switch_started_at_;
  ...
  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr legacy_mode_status_pub_;
