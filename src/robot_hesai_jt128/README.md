...
- `local_vendor_driver_config`: reuses `D:/codespace/car/ros2_ws/src/hesai_lidar_ros2/config/config.yaml`. On Jetson, `run_driver.sh` first honors `NJRH_HESAI_CONFIG_FILE`; if the historical upstream config path is absent, it falls back to the repo-owned `src/third_party/hesai_lidar_ros2_overlay/config/config.yaml` so driver-integrated restart does not depend on a temporary generated config file.
...
- `PointCloudAccelCore`: shared C++ implementation used by standalone `pointcloud_accel_axis_node` and driver-integrated `hesai_accel_driver_node`. Production local dynamic-obstacle handling uses Nav2's standard LaserScan flow: the accel scan worker publishes `/scan`, Nav2 `ObstacleLayer` marks and raytraces from `/scan`, and `collision_monitor` also consumes `/scan`. Before binning, each candidate return is transformed into `base_link`; the runtime YAML excludes XY endpoints inside `x=-0.39..0.39m`, `y=-0.36..0.36m` (inclusive, user-requested on 2026-09-21). This is a sensing exclusion, not the padded planning footprint: any real obstacle return inside it is also removed. The central sides now have no visible band inside StopZone; front/rear retain 8 cm. Future ultrasound is not integrated. The mask also affects downstream `/flatscan`, not the full `/lidar_points` trunk. Bounds are cached at startup; file synchronization alone does not activate them. If the mask TF is unavailable, that scan is withheld instead of publishing unfiltered self returns, and status counters expose both filtered points and TF-unavailable events. The old local PointCloud2 obstacle/clearing worker is disabled by default and is not a production path. See [scope and acceptance](../robot_nav_config/docs/body_geometry_alignment.md#scan-mask-width-update-2026-09-21).
...
- `nav_cloud_preprocessor`: upstream `jt128_nav_tools` helper used by localization/mapping scan slicing only when an explicit localization/mapping preprocessor path needs it. Production local obstacle handling does not consume `/points_nav`.
- The pointcloud remap includes in-place fast paths for the validated JT128 canonical axis matrices, including the current `x=raw_y, y=-raw_x, z=raw_z` runtime matrix. Production runtime keeps `/lidar_points` as the full-density canonical trunk for mapping FAST-LIO2 and diagnostic consumers; local obstacle marking and clearing consume `/scan`, not a derived PointCloud2 branch.
