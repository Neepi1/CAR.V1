...
TEST(ApplicationRouterModuleTest, ElevatorInterlockPreemptsEveryBusinessHandler)
{
  ...
  const auto response = router.route(
    request_for("POST", "/api/v1/navigation/goal"), false);
...
TEST(ApplicationRouterModuleTest, PreservesCompleteHandlerOrderAndSingleEpoch)
{
  std::vector<std::string> calls;
  auto ports = make_ports(calls);
  ...
  EXPECT_EQ(
    ...
    (std::vector<std::string>{
      "epoch", "interlock", "system_status", "maps", "elevator", "mapping",
...
TEST(ApplicationRouterModuleTest, StopsAtFirstMatchingFeature)
{
  ...
  EXPECT_EQ(calls, (std::vector<std::string>{"epoch", "interlock", "system_status"}));
}
