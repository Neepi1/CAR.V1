...
{
  const auto value = json_number_value(text, key);
  if (
...
std::optional<RuntimeMapContext> read_runtime_map_context_file(const fs::path & path)
{
  ...
  context.transaction_id =
    json_string_value(text, "transaction_id").value_or("");
  context.asset_epoch = json_uint64(text, "asset_epoch");
  context.asset_digest =
    json_string_value(text, "asset_digest").value_or("");
  context.localizer_generation =
    json_uint64(text, "localizer_generation");
  context.explicit_relocalization_sequence =
    json_uint64(text, "explicit_relocalization_sequence");
