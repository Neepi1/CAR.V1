...
struct DocumentMigration
{
  YAML::Node document;
  bool migrated{false};
  std::uint32_t source_schema_version{0U};
};
  ...
  std::string message)
{
  const auto duplicate = std::find_if(
    ...
    [&code, &field, &message](const ValidationIssue & issue) {
  ...
  const robot_elevator_manager::TopologyIssueCode code)
{
  ...
  switch (code) {
    ...
    case Code::kMissingPanelSide:
      return "MISSING_PANEL_SIDE";
    case Code::kUnsupportedSchema:
...
std::uint32_t configuration_schema_version(const YAML::Node & root)
{
  if (!root || !root.IsMap()) {
    return 0U;
  }
  const auto value = node_number(root["schema_version"]);
  if (!value || (*value != 1.0 && *value != 2.0 && *value != 3.0)) {
    return 0U;
  }
  return static_cast<std::uint32_t>(*value);
}

DocumentMigration migrate_v1_document_to_v2(const YAML::Node & source)
{
  DocumentMigration result;
  result.source_schema_version = configuration_schema_version(source);
  result.document = YAML::Clone(source);
  if (result.source_schema_version != 1U ||
    !result.document || !result.document.IsMap())
  {
    return result;
  }

  result.document["schema_version"] = 2;
  YAML::Node elevators = result.document["elevators"];
  if (elevators && elevators.IsSequence()) {
      ...
      elevator_index < elevators.size(); ++elevator_index)
    {
        ...
        floor_index < floors.size(); ++floor_index)
      {
        ...
        if (!floor || !floor.IsMap()) {
          continue;
        }
        floor.remove("threshold");
        YAML::Node poses = floor["poses"];
        if (!poses || !poses.IsMap()) {
          continue;
        }

        YAML::Node landing;
        for (const auto * role : {"hall_wait", "doorway", "exit"}) {
          const YAML::Node candidate = poses[role];
          if (candidate.IsDefined()) {
            landing = YAML::Clone(candidate);
            break;
          }
        }
        poses.remove("hall_wait");
        poses.remove("doorway");
        poses.remove("exit");
        poses.remove("landing");
        if (landing.IsDefined()) {
          poses["landing"] = landing;
        }
      }
    }
  }
...
double absolute_yaw_error(const double left, const double right)
{
  return std::abs(normalized_yaw(left - right));
}

const PoseRecord * pose_for_role(
  const FloorRecord & floor,
  const PoseRole role)
{
  const auto pose = std::find_if(
    floor.poses.cbegin(), floor.poses.cend(),
    [role](const PoseRecord & candidate) {return candidate.role == role;});
  return pose == floor.poses.cend() ? nullptr : &*pose;
}

void validate_reverse_entry_geometry(
  ValidationResult & result,
  const FloorRecord & floor,
  const std::string & floor_path)
{
  constexpr double kHalfTurnToleranceRad = 0.20;
  constexpr double kAlignedYawToleranceRad = 0.10;
  constexpr double kMinimumReverseTravelM = 0.10;
  constexpr double kMaximumAxisResidualM = 0.15;

  const auto * hall_call = pose_for_role(floor, PoseRole::kHallCall);
  const auto * landing = pose_for_role(floor, PoseRole::kLanding);
  const auto * cabin = pose_for_role(floor, PoseRole::kCabin);
  const auto * cabin_panel = pose_for_role(floor, PoseRole::kCabinPanel);
  if (!hall_call || !landing || !cabin || !cabin_panel) {
    return;
  }

  if (absolute_yaw_error(landing->yaw, hall_call->yaw + kPi) >
    kHalfTurnToleranceRad)
  {
    append_issue(
      result, "REVERSE_ENTRY_LANDING_YAW_INVALID",
      floor_path + ".poses.landing.yaw",
      "landing yaw must face approximately 180 degrees away from the hall-call yaw");
  }
  if (absolute_yaw_error(cabin->yaw, landing->yaw) >
    kAlignedYawToleranceRad)
  {
    append_issue(
      result, "REVERSE_ENTRY_CABIN_YAW_INVALID",
      floor_path + ".poses.cabin.yaw",
      "cabin yaw must match landing yaw for straight reverse entry");
  }

  const double landing_dx = cabin->x - landing->x;
  const double landing_dy = cabin->y - landing->y;
  const double reverse_longitudinal =
    std::cos(landing->yaw) * landing_dx +
    std::sin(landing->yaw) * landing_dy;
  const double reverse_lateral =
    -std::sin(landing->yaw) * landing_dx +
    std::cos(landing->yaw) * landing_dy;
  if (reverse_longitudinal > -kMinimumReverseTravelM ||
    std::abs(reverse_lateral) > kMaximumAxisResidualM)
  {
    append_issue(
      result, "REVERSE_ENTRY_AXIS_INVALID",
      floor_path + ".poses.cabin",
      "cabin must lie straight behind landing in the commissioned robot heading");
  }

  if (absolute_yaw_error(cabin_panel->yaw, cabin->yaw) >
    kAlignedYawToleranceRad)
  {
    append_issue(
      result, "CABIN_PANEL_YAW_INVALID",
      floor_path + ".poses.cabin_panel.yaw",
      "cabin-panel yaw must match cabin yaw for pure lateral approach");
  }
}
  ...
  const ElevatorFloorAssetResolver & resolver)
{
  ...
  result.schema_version = configuration_schema_version(root);
  if (result.schema_version == 0U) {
    append_issue(
      result, "UNSUPPORTED_SCHEMA_VERSION", "schema_version",
      "schema_version must be 1, 2, or 3");
  }
  const bool legacy_v1 = result.schema_version == 1U;
  const bool simplified_v2 = result.schema_version == 2U;
  const bool reverse_entry_v3 = result.schema_version == 3U;
  ...
  for (std::size_t elevator_index = 0; elevator_index < elevators.size(); ++elevator_index) {
    ...
    topology.schema_version = result.schema_version;
    ...
    if (!floors || !floors.IsSequence()) {
      ...
      for (std::size_t floor_index = 0; floor_index < floors.size(); ++floor_index) {
        ...
        if (reverse_entry_v3) {
          const auto read_panel_side =
            [&result, &floor_path, &floor_node](
              const char * field) -> PanelSide
            {
              const auto text = node_string(floor_node[field]);
              const auto side = text ?
                robot_elevator_manager::panel_side_from_string(*text) :
                std::nullopt;
              if (!side.has_value()) {
                append_issue(
                  result, "MISSING_PANEL_SIDE",
                  floor_path + "." + field,
                  std::string(field) + " must be LEFT or RIGHT");
                return PanelSide::kUnknown;
              }
              return *side;
            };
          floor.hall_call_panel_side =
            read_panel_side("hall_call_panel_side");
          floor.cabin_panel_side =
            read_panel_side("cabin_panel_side");
          topology_floor.hall_call_panel_side =
            floor.hall_call_panel_side;
          topology_floor.cabin_panel_side = floor.cabin_panel_side;
        }
        ...
        if (!poses || !poses.IsMap()) {
          append_issue(
            ...
            reverse_entry_v3 ?
            "poses must contain hall_call, landing, cabin, and cabin_panel" :
            (simplified_v2 ?
            "poses must be an object containing hall_call, landing, and cabin" :
            "poses must be an object containing all five legacy roles"));
        } else if (legacy_v1 || simplified_v2 || reverse_entry_v3) {
          const std::set<std::string> known_roles = legacy_v1 ?
            std::set<std::string>{
            "hall_call", "hall_wait", "doorway", "cabin", "exit"} :
            (reverse_entry_v3 ?
            std::set<std::string>{
            "hall_call", "landing", "cabin", "cabin_panel"} :
            std::set<std::string>{"hall_call", "landing", "cabin"});
          for (const auto & entry : poses) {
            const auto name = node_string(entry.first).value_or("");
            if (known_roles.count(name) == 0U) {
              const bool legacy_role_in_v2 =
                simplified_v2 &&
                (name == "hall_wait" || name == "doorway" || name == "exit");
              append_issue(
                result,
                legacy_role_in_v2 ?
                "LEGACY_POSE_ROLE_FORBIDDEN" : "UNKNOWN_POSE_ROLE",
                floor_path + ".poses." + name,
                legacy_role_in_v2 ?
                "schema v2 replaces hall_wait, doorway, and exit with landing" :
                "pose role is not part of the elevator configuration contract");
            }
          }
          for (const auto role :
            robot_elevator_manager::required_pose_roles(result.schema_version))
          {
            const auto role_name = robot_elevator_manager::to_string(role);
        ...
        if (legacy_v1) {
          ...
          if (threshold_ok) {
            threshold_ok =
              read_point(threshold["left"], parsed_threshold.left) &&
              read_point(threshold["right"], parsed_threshold.right) &&
              read_point(
              threshold["cabin_reference"],
              parsed_threshold.cabin_reference);
            const auto clearance = node_number(threshold["clearance_m"]);
            const auto jamb_clearance =
              node_number(threshold["jamb_clearance_m"]);
            ...
            parsed_threshold.clearance_m = clearance.value_or(0.05);
            parsed_threshold.jamb_clearance_m =
              jamb_clearance.value_or(0.05);
          }
          if (!threshold_ok) {
            append_issue(
            ...
            topology_floor.threshold = parsed_threshold;
          }
        } else if ((simplified_v2 || reverse_entry_v3) && threshold.IsDefined()) {
          append_issue(
            result, "LEGACY_THRESHOLD_FORBIDDEN", floor_path + ".threshold",
            "schema v2+ does not contain operator-authored threshold geometry");
        }

        if (reverse_entry_v3) {
          validate_reverse_entry_geometry(result, floor, floor_path);
        }
              ...
              snapshot->asset_digest.empty())
            {
              ...
              if (floor.threshold) {
                ...
                for (const auto & point : threshold_points) {
                  if (!point_inside_map(
                      point.second.x, point.second.y, *snapshot->map_info))
                  {
                    append_issue(
  ...
  configuration["schema_version"] = result.schema_version;
  ...
  topology["building_id"] = expected_building_id;
  YAML::Node internal;
  internal["schema_version"] = result.schema_version;
  ...
  for (const auto & elevator : result.elevators) {
    ...
    for (const auto & floor : elevator.floors) {
      ...
      if (result.schema_version == 3U) {
        const auto hall_side =
          robot_elevator_manager::to_string(floor.hall_call_panel_side);
        const auto cabin_side =
          robot_elevator_manager::to_string(floor.cabin_panel_side);
        configuration_floor["hall_call_panel_side"] = hall_side;
        configuration_floor["cabin_panel_side"] = cabin_side;
        topology_floor["hall_call_panel_side"] = hall_side;
        topology_floor["cabin_panel_side"] = cabin_side;
      }
      ...
      if (floor.threshold) {
        const auto write_threshold = [&floor](YAML::Node target) {
            ...
            target["cabin_reference"].push_back(
              floor.threshold->cabin_reference.x);
            target["cabin_reference"].push_back(
              floor.threshold->cabin_reference.y);
            target["clearance_m"] = floor.threshold->clearance_m;
            target["jamb_clearance_m"] =
...
std::uint32_t draft_revision_schema_version(const std::string & value)
{
  ...
  const bool digest_valid = std::all_of(
  ...
  return digest_valid ? schema_version : 0U;
}
  ...
  const std::string & validation)
{
  if (schema_version != 1U && schema_version != 2U && schema_version != 3U) {
    throw std::invalid_argument(
            "elevator draft schema version must be 1, 2, or 3");
  }
  return "draft-v" + std::to_string(schema_version) + "-" + fixed_hex(
    fnv1a64(
      building_id + "\n" + configuration + "\n" + validation),
    16);
}
  ...
  const std::string & revision)
{
  ...
  const auto canonical_configuration =
    canonical_configuration_json(configuration_root);
  const auto schema_version = draft_revision_schema_version(revision);
  if (configuration_schema_version(configuration_root) != schema_version ||
    canonical_configuration != bundle.configuration ||
    draft_revision_for(
      schema_version, building_id, bundle.configuration, bundle.validation) !=
    revision)
  {
    throw std::runtime_error(
  ...
  const std::optional<std::uint64_t> expected_generation = std::nullopt)
{
  ...
  const auto content_schema_version =
    configuration_schema_version(configuration_root);
  ...
  if (manifest_parent != current_parent ||
    ...
    (source_draft &&
    draft_revision_schema_version(*source_draft) != content_schema_version) ||
    ...
    try {
      ...
      if (!fs::exists(config_root)) {
        ...
        return make_reply(
          ...
          "\"migration_required\":false,\"legacy_read_only\":false,"
          "\"source_schema_version\":null,\"configuration_schema_version\":null,"
      ...
      if (!request.release_id.empty()) {
        ...
        const auto source_schema_version =
          configuration_schema_version(configuration);
        ...
        fields << "\"building_id\":" << json_string(request.building_id) << ","
               ...
               << ",\"legacy_read_only\":"
               << (source_schema_version == 1U ? "true" : "false")
      ...
      bool legacy_read_only = false;
      if (draft.exists) {
        const auto bundle = load_verified_draft_bundle(
          config_root, request.building_id, draft.revision);
        const auto stored_configuration = load_strict_json(
          bundle.configuration, "elevator draft configuration");
        source_schema_version =
          configuration_schema_version(stored_configuration);
        const auto migration =
          migrate_v1_document_to_v2(stored_configuration);
        configuration =
          migration.migrated ? migration.document : stored_configuration;
        migration_required = migration.migrated;
        ...
        configuration = load_strict_json(
          bundle.configuration, "elevator release configuration");
        source_schema_version = configuration_schema_version(configuration);
        legacy_read_only = source_schema_version == 1U;
        validation = validate_document(
          request.building_id, configuration, resolver_);
      }
      ...
      fields << "\"building_id\":" << json_string(request.building_id) << ","
        ...
        (current.exists ? "\"current\"" : "null")) << ","
             ...
             << "\"legacy_read_only\":"
             << (legacy_read_only ? "true" : "false") << ","
             << "\"source_schema_version\":"
             << (configuration ? std::to_string(source_schema_version) : "null") << ","
             << "\"configuration_schema_version\":"
             << (configuration ?
        std::to_string(configuration_schema_version(configuration)) : "null") << ","
             << "\"configuration\":"
...
private:
    ...
    const fs::path & config_root)
  {
    ...
    const auto source_schema_version = configuration_schema_version(root);
    if (source_schema_version == 0U) {
      return make_reply(
        400, "UNSUPPORTED_SCHEMA_VERSION",
        "\"error\":\"configuration schema_version must be 1, 2, or 3\"");
    }
    const auto migration = migrate_v1_document_to_v2(root);
    if (migration.migrated) {
      root = migration.document;
    }
    ...
    fields << "\"building_id\":" << json_string(command.building_id) << ","
           ...
           << "\"configuration_schema_version\":"
           << validation.schema_version << ","
           << "\"configuration\":" << persisted_document << ","
    ...
    const fs::path & config_root)
  {
    ...
    const auto draft_schema = draft_revision_schema_version(draft.revision);
    const auto document_schema = configuration_schema_version(document);
    if ((draft_schema != 2U && draft_schema != 3U) ||
      document_schema != draft_schema)
    {
      return make_reply(
        422, "LEGACY_DRAFT_MIGRATION_REQUIRED",
        "\"migration_required\":true,\"source_schema_version\":1,"
        "\"configuration_schema_version\":2,\"runtime_applied\":false");
    }
    ...
    const fs::path & config_root)
  {
    ...
    if (configuration_schema_version(document) == 1U) {
      return make_reply(
        409, "LEGACY_RELEASE_READ_ONLY",
        "\"error\":\"schema v1 elevator releases are read-only; migrate the "
        "configuration to a schema v2 draft before publishing\","
        "\"legacy_read_only\":true,\"source_schema_version\":1,"
        "\"runtime_applied\":false");
    }
    auto validation = validate_document(
      command.building_id, document, resolver_);
