...
- `POST /api/v1/navigation/goal` resolves a saved pose or direct map-frame pose, waits for a fresh `map -> odom -> base_link` TF chain, sends a Nav2 `NavigateToPose` goal, and tracks the goal in a background job exposed as `navigation_goal`. If the robot has strong live dock evidence or valid non-stale latch evidence, the API first performs `/docking/undock`, triggers post-undock relocalization after odometry confirms departure, waits for that result to be accepted by `robot_localization_bridge`, waits for the post-relocalization settle barrier, and only then sends the Nav2 goal. Success and admission-failure responses include `pre_navigation_undock`, `pre_navigation_undock_detail`, and `pre_navigation_dock_check`.
- Dock/contact detection for navigation admission is explicit, not position-based. In addition to stable BMS contact and `/docking/status`, the server reads `docking_contact_latch.json`, written by stable BMS contact, docking success, manual maintenance confirmation, and undock success. `pre_navigation_dock_check.dock_contact_snapshot`, `strong_live_docked`, `latch_valid_for_auto_undock`, `docked_state_class`, `docked_evidence`, and `docked_warnings` expose this state. A stale `source=bms` latch is safety memory, not permanent docked truth; live undocked status plus stable BMS no-contact clears it before normal navigation.
...
- `GET /api/v1/navigation/state` returns the latest background navigation cancel job and `navigation_goal` state for App polling and diagnostics. After Nav2 returns, normal point goals are verified against a fresh `map -> base_link` pose and completed according to `goal_completion_policy`. `position_only` is the default and completes after XY verification without mission-layer final yaw. `pose_required` runs bounded final yaw only when needed and succeeds only after `final_pose_verified`; while `position_reached_yaw_aligning`, App clients must keep `task_complete=false` visible.
- `POST /api/v1/docking/start` resolves a saved dock contact pose, prefers a manual pre-dock pose (`predock_pose_id`, `approach_pose_id`, or the `dock_id_predock` naming convention), falls back to a geometric pre-dock offset only when no manual point exists, waits for bridge acceptance and the post-relocalization settle barrier before predock Nav2, sends Nav2 to that pose, relocalizes again after the pre-dock goal succeeds, waits for the settle barrier, verifies that the fresh `map -> base_link` pose is still near the pre-dock approach pose, then calls `/docking/start` for GS2 fine docking. When fine docking reports success, failure, or stop, the API triggers another localization pass before it returns to the final docking state so Nav2 does not resume from a pose accumulated only through non-Nav2 fine-docking motion.
...
- `POST /api/v1/navigation/goal` checks the critical Nav2 lifecycle nodes and uses a when-needed relocalization gate. Confirmed same-map runtime context plus a fresh `map -> base_link` pose skips `/global_localization/trigger`; cold or unconfirmed runtime context, stale/missing map-frame pose, controlled undock, docking transitions, or explicit `force_relocalize` still arms the bridge force-accept service, triggers `/global_localization/trigger`, waits for a fresh `/localization_result` to be accepted by `robot_localization_bridge`, waits for the post-relocalization settle barrier, and only then sends the Nav2 goal. If the lifecycle is inactive or required localization cannot refresh, the goal is rejected instead of navigating from a stale odometry-only map pose.
...
AMCL continuous-localization readiness is exposed from the runtime status file, not inferred from old logs. `GET /api/v1/status` and `GET /api/v1/navigation/state` include `amcl_state`, `amcl_ready`, `amcl_degraded`, `amcl_degraded_reason`, `amcl_process_alive`, `amcl_scan_admission_alive`, `/amcl_pose` publisher count, scan-admission status publisher count, `localization_degraded`, and `using_triggered_baseline_only`. In `shadow` mode an AMCL startup/readiness failure can continue as a visible degraded Isaac-triggered baseline; in `gated` mode the runtime reports a readiness failure instead of silently treating AMCL as active.
...
Delivery completion is policy-driven. Nav2 action success is not accepted by itself: the API always rechecks the fresh `map -> base_link` pose against `navigation_goal_position_success_tolerance_m` (`0.20 m` by default). Normal delivery defaults to `goal_completion_policy=position_only`, so verified XY sets `final_pose_verified=true` and `task_complete=true` without final yaw. Orientation-sensitive points use `pose_required`; if yaw error is above `navigation_final_yaw_tolerance_rad` (`0.05 rad`), the API runs 15 Hz mission-layer final yaw with `kp=1.2`, `min=0.06 rad/s`, `max=0.25 rad/s`, `timeout=8.0 s`, and `max_xy_drift=0.08 m`, then rechecks the fresh map pose. If final yaw exceeds the XY drift guard, the API can run one `REPOSITION_AFTER_YAW_DRIFT` retry to the original target. `pose_required` only succeeds when `final_pose_verified=true`; `position_reached` alone is not task completion. This is not implemented by tightening Nav2's yaw goal checker. The first field version publishes final yaw commands to `/cmd_vel_collision_checked`, so `robot_safety` remains the final arbiter; this intentionally exposes `final_yaw_align_bypass_collision_monitor=true` until a dedicated `/cmd_vel_nav` mux is introduced. A missed position remains a failed navigation goal, even if Nav2 falsely reports success.
