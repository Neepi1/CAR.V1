...
When the backend state is `docked`, `/docking/status` starts with `docked` or `charging`, or live BMS charging contact is fresh, `/api/v1/navigation/goal` automatically performs controlled undocking before sending the Nav2 goal. The same snapshot is available through read-only `GET /api/v1/navigation/pre_goal_check` and through `/api/v1/status` / `/api/v1/docking/state` as `pre_navigation_dock_check`. The snapshot exposes `api_bms_charging_contact`, `api_bms_charging_contact_reason`, `bms.power_supply_status`, `bms.current`, `docking.last_status`, `final_is_docked_or_charging`, `final_auto_undock_required`, and `auto_undock_reason`, so a full battery with `current=0` is still diagnosable through `POWER_SUPPLY_STATUS_FULL` or the BMS contact reason. If `robot_docking_manager` has just been started and has not received `/battery_state` yet, the API retries `/docking/undock` for `docking_undock_charging_retry_sec` while the API-side charging contact remains fresh. After `/docking/status` reports odometry-confirmed `undocked`, the API arms `/robot_localization_bridge/force_accept_next_localization`, triggers `/global_localization/trigger`, and waits until the fresh localization result is reflected in `map -> base_link`; if undocking, post-undock relocalization, or bridge acceptance fails or times out, the navigation goal is not sent. Goal responses include `pre_navigation_undock`, `pre_navigation_undock_detail`, and `pre_navigation_dock_check` so the App can show whether departure from the charger happened inside the request.
...
The script is read-only unless `--execute-goal` is supplied.
