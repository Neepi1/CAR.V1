...
- `POST /api/v1/navigation/goal` resolves a saved pose or direct map-frame pose, sends a Nav2 `NavigateToPose` goal, and tracks the goal in a background job exposed as `navigation_goal`. If the robot is docked or live charging contact is detected, the API first performs `/docking/undock`, triggers post-undock relocalization after odometry confirms departure, waits for that result to be accepted by `robot_localization_bridge`, and only then sends the Nav2 goal.
...
- `POST /api/v1/navigation/goal` checks the critical Nav2 lifecycle nodes, arms the bridge force-accept service, triggers `/global_localization/trigger`, waits for a fresh `/localization_result` to be accepted by `robot_localization_bridge`, and only then sends the Nav2 goal. If the lifecycle is inactive or localization cannot refresh, the goal is rejected instead of navigating from a stale odometry-only map pose.
- `GET /api/v1/docking/state` returns the latest docking job, `/docking/status`, `charging_contact`, and `inferred_docked` state.
...
Delivery success is position-first. Nav2 action success is not accepted by itself: the API always rechecks the fresh `map -> base_link` pose against `navigation_goal_position_success_tolerance_m`. If Nav2 or the final direct yaw alignment cannot finish only the last heading correction, but the final pose is within that position tolerance, the job reports success with `phase: "position_reached_yaw_warning"` and `final_yaw_align_blocked: true`. The direct yaw align command is still published only through `/cmd_vel_collision_checked` so `robot_safety` remains the arbiter. A missed position remains a failed navigation goal, even if Nav2 falsely reports success.

When the backend state is `docked`, `/docking/status` starts with `docked` or `charging`, or live BMS charging contact is fresh, `/api/v1/navigation/goal` automatically performs controlled undocking before sending the Nav2 goal. If `robot_docking_manager` has just been started and has not received `/battery_state` yet, the API retries `/docking/undock` for `docking_undock_charging_retry_sec` while the API-side charging contact remains fresh. After `/docking/status` reports odometry-confirmed `undocked`, the API arms `/robot_localization_bridge/force_accept_next_localization`, triggers `/global_localization/trigger`, and waits until the fresh localization result is reflected in `map -> base_link`; if undocking, post-undock relocalization, or bridge acceptance fails or times out, the navigation goal is not sent. Successful responses include `pre_navigation_undock` and `pre_navigation_undock_detail` so the App can show whether departure from the charger happened inside the request.
