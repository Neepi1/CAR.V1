...
void RangerROSMessenger::PublishStateToROS() {
  ...
  // publish motion mode
  {
    ...
    motion_msg.linear_velocity = state.motion_state.linear_velocity;
    motion_msg.angular_velocity = state.motion_state.angular_velocity;
    motion_msg.steering_angle = state.motion_state.steering_angle;
...
void RangerROSMessenger::TwistCmdCallback(geometry_msgs::msg::Twist::SharedPtr msg) {
  ...
  const auto set_motion_mode = [this](const uint8_t mode) {
    motion_mode_ = mode;
    last_commanded_motion_mode_valid_ = true;
    last_commanded_motion_mode_ = mode;
  ...
  if (zero_cmd && ShouldHoldZeroCommandInSpinningMode()) {
    set_motion_mode(MotionState::MOTION_MODE_SPINNING);
    ...
    if (msg->linear.x == 0.0 && robot_type_ == RangerSubType::kRangerMiniV1) {
      set_motion_mode(MotionState::MOTION_MODE_SIDE_SLIP);
    } else {
      set_motion_mode(MotionState::MOTION_MODE_PARALLEL);
    }
  } else {
    ...
    if (radius < robot_params_.min_turn_radius) {
      ...
      set_motion_mode(MotionState::MOTION_MODE_DUAL_ACKERMAN);
    }
  }
