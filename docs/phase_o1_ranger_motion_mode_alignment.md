...
| `src/robot_api_server/src/robot_api_server_node.cpp` | `run_final_yaw_align`, `publish_final_yaw_align_command` | No | No | N/A | No | No | Publishes pure yaw to `/cmd_vel_collision_checked` | Yes | Indirect through speed chain | No actual mode check before final yaw | Leave command path unchanged; O1 status shows desired SPINNING and actual alignment after safety/mode controller |
...
| `src/robot_docking_manager/src/docking_manager_node.cpp` | `enter_docking_motion_mode`, `publish_forced_mode` | Forces legacy mode | No desired topic directly | Publishes `crab` to `/ranger_mini3/forced_mode` | No | No | Requests docking crab mode | No | Indirect | Legacy forced inputs must keep working | Accept both legacy words and official code/name strings |
