...
Before accepting a normal `POST /api/v1/navigation/goal`, the backend checks the resident runtime context, waits for the `/navigate_to_pose` action server when sending the goal, and requires `robot_localization_bridge` to report `safe_for_goal_start=true` with no non-standby AMCL correction pending/not-ready transition. Normal goals do not call `/global_localization/trigger`, force-accept, post-relocalization settle, or synchronous Nav2 lifecycle `GetState` probes; lifecycle services are startup/diagnostic checks because they can time out while Nav2 is actually active under Jetson/FastDDS load. If the App requests `"force_relocalize": true`, or bridge localization is genuinely not ready for goal start, the backend returns a recovery-required or transition-active error and expects the App or operator to use the explicit localization trigger only when recovery is actually required. Successful responses include `pre_navigation_relocalization_requested=false`, `pre_navigation_relocalization_succeeded=true`, and `pre_navigation_relocalization_detail` for App diagnostics.

The backend treats a Nav2 action result of `succeeded` as input to API-side verification, not as business completion by itself. Normal delivery goals default to `goal_completion_policy: "pose_required"` and include the target yaw in the `NavigateToPose` action. Nav2 native `RotationShimController + SimpleGoalChecker` must complete XY and yaw before action success. After Nav2 returns, the API waits briefly for bridge `map->odom` smoothing, re-reads a fresh `map -> base_link` pose, and requires the final distance and yaw to be within configured tolerances. If AMCL/bridge correction leaves a recoverable residual, the API may resend the same `NavigateToPose` goal once; it does not publish nonzero `/cmd_vel_collision_checked` or `/cmd_vel_docking` correction commands. API-owned ordinary `final_yaw_align` speed control is disabled by default and exposed as `api_final_yaw_align_enabled=false`; it must not be assumed as part of normal delivery. Engineering tools can request `goal_completion_policy: "position_only"` only when the final heading is irrelevant. `dock_staging` is reserved for `/api/v1/docking/start` and must not be used for normal delivery goals. The App must show success only when `task_complete=true`.
