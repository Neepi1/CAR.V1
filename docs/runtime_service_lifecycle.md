...
If `NJRH_AMCL_LOCALIZATION_MODE=shadow` or `gated`, the resident navigation runtime starts `run_amcl_shadow_localization.sh` only after the initial Isaac triggered relocalization has passed the bridge and `map -> odom` gates. AMCL is not part of the Nav2 controller/planner lifecycle and does not publish TF. It subscribes to `/scan`, publishes `/amcl_pose`, and is seeded through `/robot_localization_bridge/seed_amcl_initial_pose`; `shadow` reports bridge candidates only, while `gated` accepts only small corrections. Navigation stop calls the AMCL stop helper before stopping the rest of the navigation stack.
