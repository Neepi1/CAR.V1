...
Hardware validation still needs a loaded navigation run after a full restart to confirm `taskset -pc <pid>` matches the intended service groups: pointcloud perception pipeline container on CPU5/CPU6, IMU remap/localization on CPU6, `robot_localization_bridge` on CPU7, planner/BT on CPU0, and both Nav2 lifecycle managers on CPU0/CPU1. Also confirm no `fastlio_mapping` process is resident during navigation, `/jt128/vendor/points_raw`, `/lidar_points`, `/lidar_points_nav`, `/points_nav`, and `/perception/obstacle_points` remain close to JT128 input cadence, `/perception/clearing_points` runs at the configured decimated cadence, the filter lifecycle manager reaches active before the core navigation lifecycle manager, and `controller_server` no longer misses 12 Hz under JT128 load. The local obstacle input topic is now the hidden `/_internal/lidar_points_local` intra-process branch and is not a normal production DDS topic to monitor continuously.

The full-size pointcloud trunk is latest-only after the upstream Hesai driver: `/lidar_points` is the high-density trunk used by mapping-owned FAST-LIO2, while `/lidar_points_nav` is the public localization/scan branch. Local obstacle filtering is loaded in `pointcloud_perception_pipeline.launch.py` with `use_intra_process_comms=true` and consumes the hidden `/_internal/lidar_points_local` branch inside the same `component_container_mt`; it does not subscribe back to `/lidar_points` and does not require a public full-size `/lidar_points_local` DDS stream. `/lidar_points`, `/lidar_points_nav`, `/points_nav`, and `/perception/obstacle_points` use best-effort QoS with depth `1`. `/points_nav` depends on the upstream `jt128_nav_tools/nav_cloud_preprocessor` patch at `scripts/jetson/runtime_overlay/patches/jt128_nav_tools_pointcloud_qos.patch`; the patched build is sourced from `${NJRH_JT128_NAV_TOOLS_PATCHED_OVERLAY:-${PROJECT_ROOT}/.runtime/jt128_nav_tools_overlay/install}` by `common_env.sh`. Without that overlay the preprocessor falls back to its old mixed QoS behavior and can reintroduce large-message queues. Debug commands such as `ros2 topic echo` and `ros2 topic hz` should be stopped after measurement because they still create DDS subscribers on these high-rate topics.
...
Default field mapping stops navigation mode services, keeps common services
alive, clears the transient runtime map context, then starts the slam_toolbox 2D
mapping chain. Optional formal 3D mapping may start FAST-LIO2/PGO explicitly.
