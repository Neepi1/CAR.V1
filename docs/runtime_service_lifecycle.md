...
The production LaserScan ObstacleLayer keeps obstacle marking at `0.25m` and
starts free-space raytracing at `0.20m`. The local costmap resolution is
`0.05m`; field capture and an exact integer-grid raytrace replay showed that a
`0.25m` clearing start skipped diagonal cells which then stayed lethal even
across repeated infinite-range clear observations. Moving the start inward by
one cell made every captured stale lethal cell ray-reachable. It does not add
valid lidar observations inside `0.25m`, so hardware acceptance must still
verify current obstacles in the `0.20..0.30m` region and must retain independent
near-field collision coverage.
...
Default field mapping is entered through one asynchronous transition owned by
`robot_api_server`. The API first cancels any active navigation task and
publishes zero velocity, then stops navigation/localization mode services while
keeping common services alive, clears the transient runtime map context, and
only then starts the slam_toolbox 2D mapping chain. The HTTP request returns
`202` before those bounded operations complete; `mode_transition` and
`mapping.start_job` in `/api/v1/status` expose progress. Navigation, docking,
and a second navigation stop/cancel cannot acquire a competing owner while the
transition is active. This preserves the no-overlapping-node-names rule without
making the App sequence internal stop scripts. Optional formal 3D mapping may
start FAST-LIO2/PGO explicitly.
...
`nav2_lifecycle_sequence.py` is a one-shot lifecycle client. After all requested
states are confirmed it flushes its result and exits with `os._exit`; it does
not run `destroy_node()`/`rclpy.shutdown()` on the success path. Field evidence
showed Fast DDS cleanup could hang after a lost service response even though
`map_server` was already active. The outer 90-second timeout then killed the
successful helper, which incorrectly made the localization owner exit and
clean up Nav2 plus AMCL. Process teardown safely reclaims this short-lived DDS
