...
- Final command arbitration runtime now exposes `/safety/status` and `/safety/motion_allowed` in addition to the final `/cmd_vel`, so field runtime can distinguish `ESTOP_ACTIVE`, `LOCALIZATION_INVALID`, and `COMMAND_STALE`
...
- The repo-owned standard navigation launch now instantiates `controller_server`, `behavior_server`, `velocity_smoother`, `collision_monitor`, and `lifecycle_manager_navigation` directly, with remaps that enforce the intended command path `cmd_vel_nav_raw -> cmd_vel_nav -> cmd_vel_collision_checked -> robot_safety -> /cmd_vel`
...
- The canonical JT128 ingress remains repository-owned. Runtime now prefers the compiled `robot_hesai_jt128` pointcloud and imu remap nodes and falls back to `scripts/jetson/runtime_overlay/scripts/pointcloud_axis_remap.py` plus `imu_axis_remap.py` only when the compiled binaries are unavailable
...
- The live `slam_toolbox` mapping chain keeps a repository-owned `scan_flip_republisher.py` between `/scan_raw` and `/scan`, but it defaults to pass-through. Full scan reversal is only enabled when `NJRH_SLAM2D_FLIP_SCAN=1` is set for field debugging
- `run_projected_map.sh` and `run_occupancy_grid_localization.sh` now both kill stale `scan_flip_republisher.py` processes before relaunch, so `/scan` stays single-publisher when switching between 2D mapping and Isaac relocalization
...
Dynamic avoidance uses a two-stage policy. `/perception/obstacle_points` marks the local voxel costmap, while `/perception/clearing_points` clears stale voxels. MPPI is configured for Ranger Mini 3's live `MOTION_MODE_DUAL_ACKERMAN` mode with an Ackermann motion model, so local avoidance should use forward steering arcs instead of pure in-place rotation. `collision_monitor` should only hard-stop for close body-risk returns and should otherwise slow the robot rather than suppress forward motion.
