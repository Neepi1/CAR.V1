...
| `scripts/jetson/runtime_overlay/scripts/run_pointcloud_accel_pipeline.sh` | `ipc_worker)` | Starts standalone `laser_scan_to_flatscan` | SIGINT/wait, then SIGTERM if needed | Now restarts by PID with bounded max restarts | `ipc_worker` | helper exits while parent remains alive | supervise standalone helper |
| `scripts/jetson/runtime_overlay/scripts/run_pointcloud_accel_pipeline.sh` | `nitros)` | Starts standalone compatibility helper | SIGINT/wait, then SIGTERM if needed | Same as `ipc_worker` | `nitros` guarded profile | helper exits while `/scan` remains alive | supervise standalone helper |
...
NJRH_FLATSCAN_WAIT_SEC=10
NJRH_FLATSCAN_MIN_HZ=5.0
NJRH_FLATSCAN_PARAMS="${NJRH_OVERLAY_ROOT}/config/jt128_flatscan.yaml"
```

The helper cleanup path sends SIGINT and waits, then sends SIGTERM if needed. It
does not use SIGKILL.
